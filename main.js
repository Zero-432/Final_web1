...
async function loadSales() {
	...
	total_products = total_products.filter(x => x.productLabel == 'Sale off');
	// console.log(total_products);
	$('#product_list_sales').empty();
	total_products.forEach(item => {
		let row = $('#product_list_sales');
		row.append(
			"<div class='col-4'" + '>' +
			"<div class='card' style='width: 18rem;'>" +
			`<img class='card-img-top' src="${item.media.link[0]}" alt='image'>` +
			"<div class='card-body'>" +
			`<p style="color: red;" class='card-text'>${item.productLabel}</p>` +
			`<hr style="border-top: 3px dashed #bbb;" >` +
			`<a class='a-card-title' href="detail.html?id=${item.productID}"><h5 style="font-size: 13pt;" class='card-title'>${item.productName}</h5></a>` +
			`<p class='card-text'>Màu: ${item.productColor}</p>` +
			`<b class='card-text'>Giá: ${item.promotion.textPriceAfterPromotion}&nbsp;&nbsp;&nbsp;&nbsp<span style="text-decoration: line-through;">${item.productPrice}</b>` +
			"</div>" +
			"</div>" +
			"</div>"
		)
...
$('#product_list_sales').ready(function () {
	loadSales();
})
...
function showDetail(id) {
	...
	$.getJSON(url, function (data) {
		...
		$('.product-detail__content > .sku_p .sku').text(product.productSKU);
...
async function bcProducts() {
	$('#bc__p').empty();
	let products = await getData(1);
	bc_products = [];
	while (bc_products.length < 4) {
		let item = products[Math.floor(Math.random() * products.length)];
		if (!bc_products.includes(item)) {
			bc_products.push(item);
		}
	}
	bc_products.forEach(item => {
		let row = $('#bc__p');
		row.append(
			"<div class='card' style='width: 18rem;'>" +
			`<img class='card-img-top' src="${item.media.link[0]}" alt='image'>` +
			"<div class='card-body'>" +
			`<a class='a-card-title' href="detail.html?id=${item.productID}"><h5 class='card-title'>${item.productName}</h5></a>` +
			`<p class='card-text'>Color: ${item.productColor}</p>` +
			`<p class='card-text'>Price: ${item.productPrice}</p>` +
			"</div>" +
			"</div>"
...
async function bc1Products() {
	$('#bc1__p').empty();
	let products = await getData(1);
	bc1_products = [];
	while (bc1_products.length < 4) {
		let item = products[Math.floor(Math.random() * products.length)];
		if (!bc1_products.includes(item)) {
			bc1_products.push(item);
		}
	}
	bc1_products.forEach(item => {
		let row = $('#bc1__p');
		row.append(
			"<div class='card' style='width: 18rem;'>" +
			`<img class='card-img-top' src="${item.media.link[0]}" alt='image'>` +
...
async function newProducts() {
	...
	let products = await getData(1);
	new_products = [];
	while (new_products.length < 4) {
		let item = products[Math.floor(Math.random() * products.length)];
		if (!new_products.includes(item)) {
...
async function new1Products() {
	$('#new1__p').empty();
	let products = await getData(1);
	new1_products = [];
	while (new1_products.length < 4) {
		let item = products[Math.floor(Math.random() * products.length)];
		if (!new1_products.includes(item)) {
			new1_products.push(item);
		}
	}
	new1_products.forEach(item => {
		let row = $('#new1__p');
		row.append(
			"<div class='card' style='width: 18rem;'>" +
			`<img class='card-img-top' src="${item.media.link[0]}" alt='image'>` +
